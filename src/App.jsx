...
function Home() {
  ...
  const [data, setData] = useState(); //esto se usara para mapear la data
  const [loading, setLoading] = useState(true); //si es true se muestra el loader, si es false se quita el loader
  const [dataChannel, setDataChannel] = useState();

  /*aqui funciona cada vez que se da click
  en uno de los items del nav
  y se mantiene ahi*/
  ...
  const selected = (id, nombre) => {
    ...
    //aqui en selected se recibe un nombre en este caso noticias es por defecto
    setBusqueda(nombre);
  };

  useEffect(() => {
    //si uso iconoNoticia no deja presionar en otros botones
...
function Search() {
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then((e) => {
        console.log(e);
        setSearchData(e.contents); //aqui ponemos la data a un estado para poder mapearlo(contents es un metodo)
...
/*  <div className="titulo">
          <span className="tituloName">videos de</span>
          <span className="tituloBusqueda">{parametro}</span>
        </div>*/
    ...
    <div className="channel">
      ...
      <div className="videos">
        {loading
          ...
          : data && data.contents.map((resu, ind) => {
            return (
              <div className="items" key={ind}>
                <Link to={`../video/${resu.video.videoId}}`}>
                  <div className="container-img">
                    <img
                      className="imagen"
                      src={resu.video.thumbnails[3].url}
                      alt=""
                    />
                  </div>
                </Link>
...
export default function App() {
  return (
    <Routes>
      ...
      <Route path="channel/:idChannel" element={<Channel></Channel>}></Route>
    </Routes>
  );
}
